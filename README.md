...
| Apple AirDrop | Discover, Ask, and Upload over HTTPS, plus `_airdrop._tcp`. On macOS the registration includes AWDL and the receiver binds `awdl0`. Set the iPhone to Everyone. The share-sheet name is `--name` only after macOS AirDrop is Receiving Off; otherwise the iPhone shows the Mac's computer name. | Contacts-only mode. It needs an Apple-signed identity, which is not in this project. The Mac's own AirDrop row saves into Downloads, not the Whoosh folder. Windows and Linux do not have AWDL, so an iPhone usually cannot see them. |

...
| Apple AirDrop | Discover, Ask, and Upload over HTTPS, plus `_airdrop._tcp`. On macOS the registration includes AWDL, the receiver binds `awdl0`, and Whoosh broadcasts the Everyone Bluetooth beacon with the scan-response name. | Contacts-only mode. It needs an Apple-signed identity, which is not in this project. The Mac's own AirDrop row, when macOS AirDrop is on, is still `rapportd` and saves into Downloads. Windows and Linux do not have AWDL. |

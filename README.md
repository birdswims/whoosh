...
| Apple AirDrop | Discover, Ask, and Upload over HTTPS, plus `_airdrop._tcp`. On macOS the registration includes AWDL and the receiver binds `awdl0`. The iPhone has to be set to Everyone for 10 minutes. While Whoosh is running it turns this Mac's AirDrop to Everyone if it was Contacts Only or Receiving Off, and restores the previous setting when you stop it. | Contacts-only mode for Whoosh itself. It needs an Apple-signed identity, which is not in this project. The Mac's own AirDrop row uses the computer name and saves into Downloads, not the Whoosh folder. Windows and Linux do not have AWDL, so an iPhone usually cannot see them. |
